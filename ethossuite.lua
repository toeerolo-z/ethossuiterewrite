...
function Library:CreateSettingsTab(Window)
    ...
    local function CaptureConfig()
        ...
        for Index, Item in ipairs(Collapsibles) do
            Data.Collapsed[Index] = Item.Collapsed and true or false
        end
        return Data
    end
    ...
    local function ApplyConfig(Data)
        ...
        for Index, State in ipairs(Data.Collapsed or {}) do
            local Item = Collapsibles[Index]
            if Item and Item.SetCollapsed then
                pcall(function()
...
local Automation = Window:AddCategory("AUTOMATION")

local ParryTab = Combat:AddTab("Auto Parry")
local KillTab = Combat:AddTab("Instant Kill")
local FarmTab = Automation:AddTab("Auto Farm")

local Box = ParryTab:AddGroupbox("General Features")

Box:AddToggle("HoldM1", {
    Text = "Hold M1",
    Default = true,
    Callback = function(Value)
        print("Hold M1:", Value)
...
local AutoParry = Box:AddToggle("AutoParry", {
    Text = "Auto Parry",
    Default = false,
    Description = "Parries incoming attacks automatically.",
    Callback = function(Value)
        print("Auto Parry:", Value)
    end,
})

AutoParry:AddKeybind({
    Default = Enum.KeyCode.E,
    Mode = "Toggle",
})

Box:AddSlider("ParryRange", {
    Text = "Parry Range",
    Default = 35,
    Min = 0,
    Max = 100,
...
Box:AddDropdown("Clan", {
    Text = "Select Clans",
    Values = { "Arima", "Kaneki", "Touka" },
    Default = "Arima",
    Callback = function(Value)
        print("Clan:", Value)
    end,
...
Box:AddDropdown("Modes", {
    Text = "Numbers",
    Values = { "One", "Two", "Three" },
    Default = { "One" },
    Multi = true,
    Callback = function(Value)
        print("Modes:", table.concat(Value, ", "))
    end,
})
...
local Box2 = ParryTab:AddGroupbox("More Features")

Box2:AddInput("PlayerName", {
    Text = "Player Name",
...
local SavedTable = Box2:AddTable("Saved Waypoints", {
    Columns = { "Name", "Position", "Keybind", "Orny God" },
    Rows = {
        { "adad", "8652, 244, 1446", "None", "Ou shi" },
        { "pizza", "8470, -6, -1059", "None", "Ou shi" },
    },
})

SavedTable:AddRow({ "hidden", "6743, 89, -736", "None" })

Box2:AddColorPicker("AccentColor", {
    Text = "Accent Color",
    Default = Color3.fromRGB(80, 144, 217),
...
KillTab:AddGroupbox("Instant Kill"):AddToggle("IK", {
    Text = "Enabled",
    Default = false,
})

FarmTab:AddGroupbox("Farming"):AddToggle("AutoFarm", {
    Text = "Auto Farm",
    Default = false,
})
...
task.spawn(function()
    ...
    Library:Notify({
        Title = "Heads up",
        Description = "THIS IS A LONG NOTIFICATION THIS IS A LONG NOTIFICATION THIS IS A LONG NOTIFICATION THIS IS A LONG NOTIFICATION THIS IS A LONG NOTIFICATION THIS IS A LONG NOTIFICATION ",
        Type = "Info",
        Duration = 7,
    })
    ...
    Library:Notify({
        Title = "Error",
        Description = "An example error notification.",
        Type = "Error",
        Duration = 6,
    })
end)
